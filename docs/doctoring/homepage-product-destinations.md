...
- First-party cards for `Orgmetra`, `TEPP`, `psychometrics-commons`, and `contextual-orchestrator` are listed only after a live public ownership check.
- The Naruon section includes a next-action control that opens `ContextualWisdomLab/naruon`.

Private, archived, client, or bootstrap-only repositories stay off the catalog until they are a public product a buyer can open.
...
ISO/IEC 25010 treats functional correctness and appropriateness as product quality characteristics: the named software and the opened software must be the same thing (International Organization for Standardization, 2023).

## Failure and rollback

Rollback restores the `waf-ids-ai-soc` label/href and removes the `argos`/`vooster` anchors and the four added first-party cards. Do not restore a `vooster-v2-mvp` href unless a live owned repository exists.
