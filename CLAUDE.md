...
There is no lint/build/deploy command. Deployment is automatic: pushing to `main` updates the live site. CI is the central required workflows plus GitHub's CodeQL default setup; `.github/workflows/codeql.yml` is a marker workflow that keeps `github/codeql-action` visible to Scorecard's SAST check, alongside Dependabot for GitHub Actions and a ClusterFuzzLite marker Dockerfile.
