...
- **Accessibility**: grouped interactive containers get `role="group"` + `aria-label`; the language toggle uses `aria-pressed`; focus ring convention is `2px solid var(--teal)` with `2px` offset; avoid `--gold` text on light backgrounds (fails WCAG AA contrast — use `--teal`; `--gold` is fine on dark `--ink`). Project-card titles already use `::after` as a full-card hit overlay, so a new-tab `↗` on those links must use `::before` (or another unused pseudo) and keep the empty stretch `::after`. New-tab context belongs in a `.visually-hidden` child with `data-i18n="a11y.opensNewTab"` — never put `data-i18n` on the same `<a>` as that child, and do not replace the HTML warning with a synthesized `aria-label`.
