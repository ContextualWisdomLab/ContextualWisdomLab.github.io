...
## 2024-09-10 - 불필요한 URLSearchParams 파싱 오버헤드 제거
**Learning:** 쿼리 스트링이 비어있는 경우에도 `new URLSearchParams(window.location.search)`를 인스턴스화하면 파싱 오버헤드가 발생합니다.
**Action:** `URLSearchParams`를 인스턴스화하기 전에 `window.location.search`의 존재 여부를 확인하는 조건문(truthiness check)을 추가하여 불필요한 파싱을 방지합니다.
