...
## 2024-06-28 - Removed decoding="async" from critical above-the-fold SVGs
**Learning:** While `decoding="async"` prevents image decoding from blocking the main thread, it delays rendering. For critical above-the-fold SVG images (especially LCP images like hero assets), this delay worsens the Largest Contentful Paint (LCP) metric.
**Action:** Ensure `decoding="async"` is removed from critical, above-the-fold SVGs, while retaining it on lazy-loaded images to balance performance.
