...
## 2026-08-07 - Remove decoding="async" from LCP SVG images
**Learning:** Adding `decoding="async"` to LCP (Largest Contentful Paint) images like critical above-the-fold SVGs can actually delay their rendering, degrading LCP performance metrics because the browser explicitly defers decoding them.
**Action:** Remove `decoding="async"` from important LCP images to allow synchronous decoding, while retaining it on `loading="lazy"` images that are off-screen.
