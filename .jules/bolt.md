...
## 2026-08-10 - 탭 컴포넌트 이벤트 핸들러의 DOM 참조 캐싱
**Learning:** 탭 전환 시마다 `document.getElementById`를 호출하여 패널을 찾는 것은 불필요한 DOM 탐색 비용을 발생시킵니다.
**Action:** 탭 목록 초기화 시 패널 DOM 참조를 미리 계산하여 클로저에 캐싱(`tabPanels`)함으로써 탭 활성화 성능을 개선합니다.
