...
## 2025-01-20 - 화면 밖 섹션 렌더링 최적화
**Learning:** 긴 페이지에서 화면 밖에 있는 요소들을 처음부터 렌더링하면 불필요한 성능 저하가 발생합니다. `content-visibility: auto`를 사용하면 렌더링을 지연시킬 수 있지만, 스크롤 점프를 방지하려면 `contain-intrinsic-size`와 함께 사용해야 합니다.
**Action:** 긴 페이지의 콘텐츠 섹션에 `content-visibility: auto`를 적용하고, 구형 브라우저 호환성 및 레이아웃 시프트를 위해 `contain-intrinsic-size`를 픽셀 단위 렌더링 예상 높이로 설정합니다.
