...
## 2024-09-23 - DOM API 접근 최적화
**학습:** `hasAttribute`를 먼저 호출하고 `getAttribute`를 호출하는 것은 중복된 DOM 연산을 발생시킵니다. 또한 ID 기반 선택 시 `querySelector('#id')` 보다 `getElementById('id')`가 더 빠릅니다.
**적용:** 중복된 `hasAttribute` 검사를 제거하고 `getAttribute` 결과값의 존재 여부만으로 조건 처리를 하여 성능을 개선하고, ID 선택 시 항상 `getElementById`를 사용하도록 합니다.
