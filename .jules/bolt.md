...
## 2024-09-21 - 불필요한 DOM 탐색 및 속성 검사 최적화
**학습:** `querySelector` 대신 `getElementById`를 사용하고, `hasAttribute`와 `getAttribute`의 중복 호출을 단일 `getAttribute` 호출로 대체하면 DOM 탐색 및 속성 검사 비용을 줄여 메인 스레드 성능을 개선할 수 있습니다.
**적용:** 단일 ID 검색에는 항상 `getElementById`를 우선 사용하고, 속성 값 검사 시 불필요한 존재 여부 확인 로직을 제거합니다.
