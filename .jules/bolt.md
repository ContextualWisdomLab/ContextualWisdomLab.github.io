...
## 2024-09-17 - 반복되는 DOM 조회를 피하기 위한 상태 캐싱
**Learning:** 탭 컴포넌트 이벤트 핸들러 내부에서 매번 DOM 요소를 조회(`getElementById`)하면, 탭을 클릭할 때마다 불필요한 O(N) DOM 조회가 발생하여 렌더링 성능이 저하될 수 있습니다. 정적 UI 컴포넌트에서는 대상 요소를 초기화 시점에 캐싱해두는 것이 유리합니다.
**Action:** 이벤트 핸들러 외부(초기화 시점)로 고비용의 DOM 조회를 분리하고 캐싱하여, 클릭 이벤트 루프 내에서 불필요한 탐색을 제거합니다.
