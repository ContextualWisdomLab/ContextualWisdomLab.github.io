...
## 2024-05-20 - [DOM 쿼리 및 속성 접근 최적화]
**학습:** `querySelector("#id")` 대신 `getElementById("id")`를 사용하면 ID 검색 속도가 훨씬 빠르며, `getAttribute` 호출 전 중복되는 `hasAttribute` 검사를 제거하면 불필요한 연산과 DOM 접근을 줄일 수 있음을 확인했습니다.
**적용:** 앞으로 DOM 조작 최적화 시, 특히 여러 노드를 순회하는 반복문 내에서는 가장 빠르고 효율적인 DOM 검색 및 접근 방식을 우선 적용합니다.
