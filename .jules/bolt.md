...
## 2026-08-13 - JS-DOM 경계 횡단 제거를 통한 i18n 성능 개선
**Learning:** `dataset` 속성을 통해 DOM 엘리먼트의 `data-*` 속성에 접근하는 것은 일반적인 자바스크립트 객체 속성 접근보다 느립니다. 다국어 처리와 같이 다수의 DOM 요소를 순회하는 작업에서 매번 DOM 경계를 넘나들며 속성을 읽는 것은 성능 저하를 유발할 수 있습니다.
**Action:** 초기 요소 선택 시점(`querySelectorAll`)에 DOM 노드와 필요한 속성(`getAttribute("data-i18n")`) 값을 자바스크립트 배열 객체로 한 번만 추출하여 캐싱(`Array.from(...).map(...)`)함으로써, 이후 언어 변경 시 발생하는 반복적인 JS-DOM 경계 횡단 비용을 제거합니다. 또한 `dataset.lang` 대신 `getAttribute("data-lang")`을 사용합니다.
