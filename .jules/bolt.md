...
## 2026-08-11 - 언어 전환 시 O(N) DOM 속성 읽기 비용 제거
**Learning:** 언어를 전환할 때마다 수십 개의 요소에서 `node.getAttribute`를 호출하는 것은 불필요한 DOM I/O 오버헤드를 발생시킵니다.
**Action:** 요소 초기 쿼리 시점에 각 요소의 번역 키 속성들을 자바스크립트 객체 배열에 캐싱하여, 핫 패스(언어 전환 이벤트)에서의 DOM 속성 읽기 비용을 줄입니다.
