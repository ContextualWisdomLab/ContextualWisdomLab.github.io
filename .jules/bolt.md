...
## 2024-05-18 - [DOM 속성 조회 최적화 및 선택자 최적화]
**학습:** DOM 노드에서 `hasAttribute`를 호출한 뒤 다시 `getAttribute`를 호출하는 것은 중복된 DOM 접근을 발생시킵니다. 또한 ID로 요소를 선택할 때는 `querySelector('#id')`보다 `getElementById('id')`가 더 빠릅니다.
**적용:** `hasAttribute` 검사를 제거하고 `getAttribute` 결과를 변수에 저장하여 재사용하도록 수정합니다. ID 선택자에는 `getElementById`를 사용합니다.
