...
## 2024-07-17 - SVG 이미지의 불필요한 decoding="async" 제거
**Learning:** SVG 이미지는 디코딩(decoding)이 아닌 파싱(parsing) 대상이므로 `decoding="async"` 속성이 무의미하며, LCP 이미지나 SVG 요소에서 사용하면 오히려 동기적 렌더링이 지연되어 성능(안티 패턴)을 저하시킬 수 있습니다.
**Action:** 앞으로 오프스크린이거나 지연 로딩되는 래스터 이미지에만 `decoding="async"` 속성을 적용하고 SVG 또는 중요 LCP 이미지에는 적용하지 않도록 합니다.
