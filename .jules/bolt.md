...
## 2024-11-20 - 빈 쿼리 스트링에서 URLSearchParams 파싱 오버헤드 방지
**Learning:** `window.location.search`가 비어있을 때도 `new URLSearchParams`를 호출하면 불필요한 파싱 오버헤드가 발생한다는 것을 확인했습니다.
**Action:** `URLSearchParams`를 인스턴스화하기 전에 항상 `window.location.search`의 truthiness를 먼저 확인하여 무의미한 메인 스레드 점유를 방지합니다.
