...
## 2026-08-12 - DOM 속성 업데이트 시 불필요한 레이아웃 재계산 방지
**Learning:** 컴포넌트 상태 업데이트 시 변경사항이 없는 DOM 요소에 대해 `setAttribute`나 프로퍼티 할당을 다시 호출하면, 브라우저가 불필요하게 스타일 무효화(Style Invalidation) 및 레이아웃 재계산을 수행할 수 있습니다.
**Action:** 탭 전환 로직에서 기존 값과 새 값을 비교(`tab.getAttribute("aria-selected") !== String(isSelected)`)하여 상태가 변한 요소에만 DOM 업데이트를 수행하도록 안전하게 최적화했습니다.
