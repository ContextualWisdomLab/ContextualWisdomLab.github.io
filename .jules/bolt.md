...
## 2026-07-31 - Remove decoding="async" from LCP images
**Learning:** `decoding="async"` on above-the-fold critical images (like LCP or hero images) can unintentionally delay their rendering, degrading Largest Contentful Paint (LCP) performance as the browser yields the main thread before decoding them.
**Action:** Remove `decoding="async"` from non-lazy, critical above-the-fold SVG/images to prioritize their rendering, while keeping it on off-screen lazy-loaded images.
