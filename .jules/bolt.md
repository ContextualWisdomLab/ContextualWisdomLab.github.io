...
## 2026-08-18 - Event Delegation 및 O(1) DOM 업데이트로 리스너/렌더링 오버헤드 감소
**Learning:** 여러 요소(탭, 태그 삭제 버튼 등)에 각각 개별적으로 이벤트 리스너를 달고 중첩 루프 O(n²)로 DOM 상태를 업데이트하면, 메모리 사용량이 증가하고 불필요한 DOM 접근/수정이 발생합니다.
**Action:** 부모(또는 document)에 단일 이벤트 리스너를 위임(Event Delegation)하고, 상태 변경이 일어난 요소만 O(1)로 선택하여 속성(aria-selected, hidden)을 토글하도록 최적화합니다.
