...
## 2026-09-12 - URLSearchParams 호출 축소
**Learning:** `window.location.search`가 비어있을 때 `new URLSearchParams`를 불필요하게 호출하는 것을 방지하면 단일 객체 할당과 파서 호출을 줄일 수 있습니다. 하지만 이는 DOM 렌더링이나 브라우저 스케줄링에는 영향을 주지 않는 초미세 최적화(micro-optimization)에 불과합니다.
