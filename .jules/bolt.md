...
## 2024-09-22 - 불필요한 DOM 탐색 및 속성 검사 제거로 성능 최적화
**학습:** `querySelector('#id')` 보다 `getElementById('id')`가 더 빠르게 동작하며, `getAttribute()`는 속성이 없으면 `null`을 반환하므로 미리 `hasAttribute()`로 검사하는 것은 중복된 비용을 발생시킵니다.
**적용:** 단일 요소 탐색에는 `getElementById`를 사용하고, 중복되는 속성 존재 여부 검사를 제거하여 렌더링 성능을 개선합니다.
