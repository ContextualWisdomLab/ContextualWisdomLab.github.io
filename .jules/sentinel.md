...
## 2026-09-06 - 버튼 기본 속성(type) 지정
**Vulnerability:** 명시적인 `type` 속성이 없는 `<button>` 요소는 폼(form) 내부에 위치할 경우 기본값인 `type="submit"`으로 동작하여 의도치 않은 폼 제출과 페이지 새로고침(상태 변경 요청)을 유발할 수 있습니다.
**Learning:** 접근성과 보안 측면 모두에서, 폼 제출 목적이 아닌 모든 버튼은 `type="button"`을 명시해야 예기치 않은 부작용을 방지할 수 있습니다.
**Prevention:** 새로운 버튼 구성요소를 작성하거나 추가할 때는 항상 목적에 맞는 `type` 속성을 명시적으로 선언하는 것을 원칙으로 삼아야 합니다.
