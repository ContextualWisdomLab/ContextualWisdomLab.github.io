...
## 2026-09-07 - 버튼 기본 타입 명시
**Vulnerability:** 명시적인 `type` 속성이 없는 `<button>` 엘리먼트는 폼 내부에 배치될 경우 브라우저 기본 동작으로 인해 `type="submit"`으로 취급되어 의도치 않은 폼 제출과 상태 변경을 유발할 수 있음.
**Learning:** 컴포넌트 갤러리의 버튼들은 폼과 무관하게 UI 상태 제어 목적으로 디자인되었으나, 향후 `<form>` 내부에서 재사용될 경우 보안 및 기능적 결함을 초래할 위험이 있음.
**Prevention:** 의도치 않은 폼 제출을 방지하기 위해 폼 제출용이 아닌 모든 UI용 `<button>` 엘리먼트에는 항상 `type="button"`을 명시하여 방어적 코딩(Defense in Depth) 원칙을 준수해야 함.
