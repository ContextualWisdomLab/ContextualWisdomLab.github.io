...
## 2026-09-23 - CSP 내 무효한 Trusted Types 지시어 제거
**취약점:** `<meta>` 태그를 통한 CSP 설정 내에 `require-trusted-types-for 'script'` 지시어가 포함되어 있었습니다. 이 지시어는 HTTP 응답 헤더를 통해서만 유효하며, 메타 태그에서는 브라우저에 의해 무시되어 보안에 혼선을 줄 수 있습니다.
**학습:** `require-trusted-types-for`는 HTTP 헤더로 전달되어야 하며 `<meta http-equiv="Content-Security-Policy">` 태그 내부에서는 지원되지 않음을 확인했습니다.
**예방:** CSP를 HTML 메타 태그로 정의할 때는 브라우저가 무시하는 지시어를 추가하지 않아야 하며, 향후 서버 응답 헤더로 마이그레이션 할 때 적용해야 합니다.
