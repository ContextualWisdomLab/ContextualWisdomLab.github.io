...
## 2026-08-14 - 로그 인젝션(Log Injection) 방지를 위한 동적 로깅 제거
...
## 2026-08-19 - 방어적 프로그래밍 추가
**Vulnerability:** DOM 요소를 조작하는 스크립트에서 요소 존재 여부를 확인하지 않아 잠재적인 런타임 에러(Unhandled Exception)가 발생할 수 있음.
**Learning:** `document.getElementById`나 `closest`와 같은 DOM 쿼리 함수는 요소를 찾지 못하면 `null`을 반환합니다. 이를 바로 조작하려고 하면 에러가 발생하여 전체 스크립트 실행이 중단될 수 있습니다. 정적 사이트에서는 이러한 에러가 사용자 인터랙션을 마비시키는 DoS(Denial of Service) 형태의 취약점으로 이어질 수 있습니다 (하지만 이 경우 백엔드가 없는 단순 클라이언트 UI 오류이므로 보안 취약점이라기보다는 방어적 프로그래밍 부재입니다).
**Prevention:** DOM 요소를 쿼리한 후에는 반드시 반환값이 `null`이 아닌지 확인하는 Null Check 로직을 추가하여 스크립트가 우아하게 저하(graceful degradation)되도록 방어적 프로그래밍을 실천해야 합니다.
