...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" '"mcp"' "opencode config declares MCP servers"
	assert_file_contains "$workflow_file" '"codegraph"' "opencode config declares the CodeGraph MCP server"
	assert_file_contains "$workflow_file" '"deepwiki"' "opencode config declares the DeepWiki MCP server"
	assert_file_contains "$workflow_file" '"context7"' "opencode config declares the Context7 MCP server"
	assert_file_contains "$workflow_file" '"web_search"' "opencode config declares the web search MCP server"
	...
	assert_file_contains "$workflow_file" '"deepseek/deepseek-v3-0324"' "opencode config defines DeepSeek V3 fallback"
	assert_file_contains "$workflow_file" '"context": 128000' "opencode config uses the GitHub Models GPT-5 200k context window"
	assert_file_contains "$workflow_file" '"output": 4096' "opencode config uses the GitHub Models GPT-5 100k output window"
	assert_file_not_contains "$workflow_file" "gpt-4.1" "opencode config must not define GPT-4.1 fallback"
	assert_file_not_contains "$workflow_file" "gpt-5-chat" "opencode config must not define unavailable GPT-5 chat fallback"
	assert_file_not_contains "$workflow_file" "gpt-5-mini" "opencode config must not define unavailable GPT-5 mini fallback"
}
