...
def _is_valid_findings(findings: Any, result: str) -> bool:
    """Validate the findings array matches the result state."""
...
def valid_control(
    ...
    *,
    ...
    if not _is_valid_metadata(
        value, expected_head_sha, expected_run_id, expected_run_attempt
    ):
        return None

    result = value["result"]
    findings = value.get("findings")
    if not _is_valid_findings(findings, result):
        return None
