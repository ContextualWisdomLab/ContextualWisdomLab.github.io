...
## 2023-10-25 - [버튼이 아닌 a 태그 외부 링크 표시 추가]
**학습:** 사용자는 호버링이나 스크린 리더를 사용하지 않으면 링크가 새 탭에서 열릴지 알 수 없습니다. 시각적 단서는 이를 명확하게 합니다. a 태그의 data-i18n 속성은 내부 span으로 이동해야 합니다.
**적용:** 버튼처럼 보이지 않는 모든 외부 링크에 대해 인라인 span `<span aria-hidden="true"> ↗</span>`을 추가했습니다. 텍스트 교체 시 표시 기호가 사라지는 것을 방지하기 위해 data-i18n을 내부 span으로 이동했습니다.
