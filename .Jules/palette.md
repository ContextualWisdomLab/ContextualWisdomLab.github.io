...
## 2024-06-25 - 외부 링크 시각 표시 개선
**학습:** `target="_blank"`인 외부 링크에 새 창에서 열린다는 시각적 표시(예: ↗)를 추가하면 UX가 향상됩니다. 하지만 다국어 번역 시스템(`data-i18n`)을 사용하는 경우, 표시용 요소를 직접 텍스트 노드 옆에 두면 번역 스크립트가 요소 전체를 덮어씌울 수 있습니다.
**적용:** 텍스트 부분만 `<span>`으로 감싸 `data-i18n`을 적용하고, 스크린 리더에서 불필요하게 두 번 읽히지 않도록 `<span aria-hidden="true">↗</span>`를 추가하는 방식으로 구조를 변경해야 합니다. 버튼 UI를 망치지 않게 버튼에는 적용하지 않도록 합니다.
