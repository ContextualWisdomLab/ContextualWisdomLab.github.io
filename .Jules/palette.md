...
## 2024-05-24 - [External link indicator]
**학습:** 모든 외부 링크(target="_blank")에 시각적 표시가 누락되어 있습니다. 이는 스크린 리더 사용자에게는 텍스트(예: "새 창에서 열림")로 제공되지만, 시각적 사용자에게는 외부 링크임을 인지시키기 위해 화살표 아이콘(↗) 같은 작은 UI 표시가 필요합니다.
**적용:** 외부 링크 텍스트의 끝부분에 작은 시각적 화살표(`↗`)를 추가하여 외부로 이동하는 링크임을 시각적으로 명확히 전달하도록 개선합니다. 버튼에는 화살표를 넣지 않습니다.
