...
## 2023-10-25 - [외부 링크 시각적 표시 추가 시 레이아웃 깨짐 현상과 번역 스크립트 충돌]
**학습:** [CSS 커스텀 클래스를 추가하지 않고 시각적 요소를 추가할 때(예: `↗`), 단순히 띄어쓰기와 함께 추가하면 요소가 줄바꿈되어 레이아웃이 깨질 수 있습니다. 또한, `data-i18n`이 적용된 요소의 직계 자식으로 추가하면 클라이언트 사이드 번역 스크립트가 요소 내용을 덮어씌워 아이콘이 사라지는 문제가 발생합니다.]
**적용:** [줄바꿈을 방지하기 위해 `&nbsp;`를 사용하여 인라인 텍스트 래핑 문제를 방지합니다. 번역 스크립트와의 충돌을 피하기 위해 `data-i18n` 속성을 내부 `<span>`으로 이동시키고, 추가하는 시각적 요소는 그 `<span>`과 형제(sibling) 관계로 배치해야 합니다.]
