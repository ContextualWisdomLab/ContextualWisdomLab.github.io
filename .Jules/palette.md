...
## 2026-07-30 - 프로젝트 카드 클릭 영역 확장
**Learning:** 텍스트 링크에만 의존하면 마우스나 터치 환경에서 클릭하기 불편할 수 있다. 시맨틱 HTML(텍스트 <a> 태그)을 유지하면서 `::after` 가상 요소를 사용하면 카드의 클릭 영역 전체를 확장할 수 있다.
**Action:** 카드 형태의 UI에서 시맨틱 구조를 유지하면서 클릭 영역을 확장하기 위해 `::after`와 `inset: 0`을 활용하는 기법(Fitts's Law)을 적용한다.
