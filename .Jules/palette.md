...
## 2025-02-15 - 외부 링크 시각적 표시 추가
**Learning:** `target="_blank"`로 새 창에서 열리는 외부 링크에 시각적인 화살표 표시(`\2197`)를 추가하면, 시각 장애인 및 일반 사용자 모두에게 현재 맥락을 벗어난다는 것을 명확히 알려주어 접근성과 UX가 크게 향상됨을 확인했습니다.
**Action:** 버튼과 같이 이미 시각적 경계나 아이콘이 있는 요소를 제외하고, `a[target="_blank"]:not(.button)::after` CSS 가상 요소를 사용하여 외부 링크에 화살표를 일관되게 추가합니다.
