...
## 2024-05-20 - [외부 링크 시각적 지시자 및 번역 호환성 적용]
**Learning:** 클라이언트 측 번역(i18n)이 적용된 요소의 직계 자식으로 DOM 노드를 추가하면 번역 스크립트가 `textContent`를 덮어쓰면서 추가된 요소가 소멸되는 문제가 발생합니다.
**Action:** 이를 방지하기 위해 텍스트 노드를 별도의 `<span>`으로 감싸 `data-i18n` 속성을 부여하고, 추가되는 시각적 지시자 요소는 그 옆 형제 노드로 배치해야 합니다. 또한 기존 클래스에 영향을 주지 않고 새로운 CSS 추가를 지양하는 규칙을 준수해야 합니다.
