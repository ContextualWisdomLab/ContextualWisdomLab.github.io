...
def test_images_decode_without_blocking_rendering():
    """Only lazy-loaded site images opt into asynchronous decoding."""
    parser = _ImageParser()
    ...
    non_lazy_images = [img for img in parser.images if img.get("loading") != "lazy"]
    assert all(image.get("decoding") != "async" for image in non_lazy_images)
