...
class _ImageParser(HTMLParser):
    """Collect image attributes from the static homepage."""
    ...
    def handle_starttag(
        ...
        """Record each ``img`` element while parsing the homepage."""
        if tag == "img":
...
def test_eager_images_leave_decoding_to_the_user_agent() -> None:
    """Eager images use the standards-defined default ``auto`` decoding hint."""
    eager_images = [
        image for image in _homepage_images() if image.get("loading") != "lazy"
    ]

    assert eager_images, "the first viewport must contain eager images"
...
def test_lcp_candidate_is_eager_and_high_priority() -> None:
    """The declared LCP candidate is eagerly fetched without forced async paint."""
    lcp_candidates = [
        image
