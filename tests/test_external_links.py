...
def test_external_links_have_accessible_titles():
    """All target=_blank links must have aria-describedby, title, and i18n title attributes."""
    ...
    for attrs in parser.links:
        if attrs.get("target") == "_blank":
            ...
            assert attrs.get("aria-describedby") == "new-window-desc", f"External link missing aria-describedby: {attrs}"
