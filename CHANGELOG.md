...
## [Unreleased]
- **성능 개선**: `i18n.js` 파일에서 `dataset` 객체 대신 `getAttribute()` 메서드를 사용하여 data-* 속성을 읽도록 변경하여, 언어 변경 시 많은 DOM 요소를 순회할 때 발생하는 성능 저하(Proxy 생성 및 변환 오버헤드)를 해결하고 메인 스레드 블로킹 시간을 단축했습니다.
