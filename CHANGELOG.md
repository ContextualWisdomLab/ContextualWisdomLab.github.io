...
## [Unreleased]
- **성능 개선**: `i18n.js`에서 DOM 요소 탐색 시 `querySelector` 대신 `getElementById`를 사용하여 조회 속도를 높이고, 반복적인 `node.dataset.i18n` (Proxy 객체) 참조 오버헤드를 줄이기 위해 번역 노드와 키를 메모리에 객체 배열로 캐싱하여 언어 전환 시 처리 성능을 개선했습니다.
