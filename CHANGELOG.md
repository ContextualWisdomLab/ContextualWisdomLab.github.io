...
- **UX/접근성 개선**: 새 탭으로 열리는 모든 홈페이지 링크에 시각적 `↗` 표시와 문서 트리의 새 탭 안내를 추가합니다. 프로젝트 카드는 기존 카드 전체 클릭 영역(`::after` 오버레이)을 유지하고 제목 앞 `::before`에 `↗`를 두며, 정적 계약 테스트가 `noopener noreferrer`, 가시 라벨, 한·영 안내 문자열, 오버레이 충돌 방지를 검증합니다.
